...
function parser(workbooks) {
  ...
  results[1] = results[1].concat(parseWorksheet(workbooks[1].worksheets[0], [
  ...
  results[1] = results[1].concat(parseWorksheet(workbooks[1].worksheets[1], [
  ...
  results[1] = results[1].concat(parseWorksheet(workbooks[1].worksheets[2], [
  ...
  results[1] = results[1].concat(parseWorksheet(workbooks[1].worksheets[3], [
  ...
  results[1] = results[1].concat(parseWorksheet(workbooks[1].worksheets[4], [
  ...
  results[1] = results[1].concat(parseWorksheet(workbooks[1].worksheets[5], [
    "textcode","no of subtexts","category","wordcount","free comments","date of recording","place of recording",
  ...
  results[2] = results[2].concat(parseWorksheet(workbooks[2].worksheets[0], [
  ...
  results[2] = results[2].concat(parseWorksheet(workbooks[2].worksheets[1], [
  ...
  results[2] = results[2].concat(parseWorksheet(workbooks[2].worksheets[2], [
  ...
  results[2] = results[2].concat(parseWorksheet(workbooks[2].worksheets[3], [
  ...
  results[3] = results[3].concat(parseWorksheet(workbooks[3].worksheets[0], [
  ...
  results[3] = results[3].concat(parseWorksheet(workbooks[3].worksheets[1], [
  ...
  results[3] = results[3].concat(parseWorksheet(workbooks[3].worksheets[2], [
...
function parseWorksheet(sheet, columns, speaker_cols, gen_text_id = (sheet) => (sheet.columns[columns.indexOf("textcode")].values[rowidx])) {
  ...
  for (let rowidx = 2; rowidx < sheet.rowCount; rowidx++) {
    ...
    for (let colidx of Array(sheet.columnCount).keys()) {
      ...
      if (colname && colname.startsWith("ignore__")) {
        ...
        if (colname === "wordcount") {
          let wordcount = sheet.columns[colidx].values[rowidx];
          if (typeof(wordcount) === "string") {
            result["wordcount"] = wordcount.replace(/ ?words/, "").trim();
          } else if (typeof(wordcount) === "number") {
            result["wordcount"] = String(wordcount)
          }
        } else {
        ...
        if (existingSpeakerIndex < 0) {
          result["ldac:speaker"].push({
            ...
            "@type": "Person",
            "role": "speaker",
            "name": `${textcode} Speaker ${speaker_letter.toUpperCase()}`
          })
          existingSpeakerIndex = result["ldac:speaker"].length-1
        }

        if (colname === "age") {
          let age = sheet.columns[colidx].values[rowidx]
          if (age) {
            if (typeof(age) === "number") {
              age = String(age)
            }
            // some values for age are 4 digits (like `1825`); these are missing a hyphen in the middle
            age = age.replace("'", "").replace(/(\d{2})(\d{2})/, "$1-$2")
            result["ldac:speaker"][existingSpeakerIndex][colname] = age;
          }
        }
    ...
    result["ldac:speaker"] = result["ldac:speaker"].filter(speaker => {
      ...
      return Object.entries(speaker).some(([k, v]) => !k.startsWith("@") && k !== "role" && v)
    })
