...
function parseWordcount(wordcount) {
  if (typeof(wordcount) === "string") {
    return wordcount.replace(/ ?words/, "").trim();
  } else if (typeof(wordcount) === "number") {
    return String(wordcount)
  }
}

function coerceNumberToString(n) {
  if (typeof(n) !== 'number') {
    throw new Error(`tried coerceNumberToString on something that wasn't a number: ${n}`)
  } else {
    return String(n)
  }
}
...
function w1b(chunk, col = defaultcolumns) {
  ...
  // TODO: date parsing
  const date = []
  const wordcount = chunk[0][col.wordcount] ? [
    parseWordcount(chunk[0][col.wordcount])
  ] : []
  ...
  for (let row of chunk.slice(1)) {
    ...
    if (row[col.wordcount]) wordcount.push(parseWordcount(row[col.wordcount]));
  ...
  return {
    ...
    title,
    authors: [...authors],
    wordcount: parseWordcount(wordcount[0]),
    subtitles: [...subtitles],
    date,
  }
}
...
function w2d(chunk, col = defaultcolumns) {
  ...
  if (chunk[0][col.textcode] === undefined) {
    ...
    result = {
      ...
      wordcount: parseWordcount(chunk[0][col.wordcount]),
      credit,
    }
  }
  return result
...
let w2e = (chunk) => w1b(chunk, {
  ...
  date: 223,
  wordcount: 224,
  author: 225 // no author col but column 225 is blank
});
